...
local opt = lapp([[
Required parameters
   --src            (default 'toy')     path for source image root directory
   --dst            (default 'aug')     path for destination image root directory
...
local function augment_image(dst, src, dim, inner_crop, offset)
   local crop_mode = (inner_crop and ('^'..dim)) or tostring(dim)
   local outer_crop = not inner_crop
   ...
   -- augment images
   local flip = opt.flip
   local degree = opt.degree
   local transImgInt = opt.transImgInt
   if flip == true then
      hflip_img(dst, y)
   end
   if degree > 0 then
      for i=1, opt.num_rotate do
         rotate_img(flip, dst, degree * i, y)
      end
   end
   if transImgInt > 0 then
...
local function create_dataset(arg)
   ...
   for loc, dirs, files in dir.walk(src_root, false, false) do
      ...
      for i = 1, #files do
         ...
         -- augment only for trainig set
         if mode == training_dir then
            local dst = path.join(dst_root, mode, dst_path, path.basename(files[ shuffle[i] ]))
            s = pcall(function() augment_image(dst, src, dim, inner_crop, offset) end)
         else
            local dst = path.join(dst_root, mode, dst_path, path.basename(files[ shuffle[i] ]))
