...
local function check_src(src)

   local formatPipe = io.popen('identify ' .. src)
   local format -- can be nill, if next line fails
   pcall(function() format = formatPipe:read():split('%s')[2] end)
   formatPipe:close()

   if format == 'JPEG' and paths.basename(src):lower():match('.jpe?g') then
      return src

   elseif format == 'PNG' and paths.basename(src):lower():match('.png') then
      return src

   elseif format == 'JPEG' and paths.basename(src):lower():match('.png') then
      local newFileName = paths.basename(src):lower():gsub('.png', '.jpg')
      local newSrc = paths.concat(paths.dirname(src), newFileName)
      print('Renaming ' .. src .. ' -> ' .. newFileName)
      os.execute('mv ' .. src .. ' ' .. newSrc)
      return newSrc

   elseif format == 'PNG' and paths.basename(src):lower():match('.jpe?g') then
      local newFileName = paths.basename(src):lower():gsub('.jpe?g', '.png')
      local newSrc = paths.concat(paths.dirname(src), newFileName)
      print('Renaming ' .. src .. ' -> ' .. newFileName)
      os.execute('mv ' .. src .. ' ' .. newSrc)
      return newSrc

   else -- it's a GIF or other crap
      print('Killing ' .. src)
      os.execute('rm ' .. src)
      return false
...
local function create_dataset(arg)
   ...
   local corrupted = io.open('corrupted.txt', 'w')
   local corrupted_error = io.open('corrupted_error.txt', 'w')
   for dirs in paths.iterdirs(src_root) do

      -- (1) get src/dst path
      print('Processing '..dirs)
      local loc = paths.concat(src_root, dirs)
      local dst_path = dirs


      -- (2) create dst directories (train/val)
      ...
      -- (3) resize img and split into train/val sets
      local pipe = io.popen('ls '..loc..' | wc -l')
      --nb_files = math.min(11e3, tonumber(pipe:read()))
      nb_files = tonumber(pipe:read())
      pipe:close()

      local i = 1
      for file in paths.iterfiles(loc) do
         -- train/val directory selector based on shuffled index
         local mode = i <= math.min(ratio*nb_files, 1e3) and validation_dir or training_dir

         -- specify each src/dst of images and resize
         local src = path.join(loc, file)
         local dst = path.join(dst_root, mode, dst_path, file)
         src = check_src(src)

         if src then
            local s, e = pcall(function() resize_image(dst, src, dim, inner_crop, offset) end)
            if not s then
               corrupted:write(src..'\n')
               corrupted_error:write(e..'\n')
               corrupted:flush()
               corrupted_error:flush()
            else
               i = i + 1
         ...
         xlua.progress(i, nb_files)

         -- jump to next directory
         --if i > nb_files then break end
      end

      collectgarbage()

      -- (4) force to have at least one sample in validation set (produce duplicated sample)
